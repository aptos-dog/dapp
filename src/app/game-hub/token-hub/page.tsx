  ...
  try {
    ...
    if (m) {
      try {
        ...
        throw new Error(
          String(serverMsg || "Server returned non-JSON response")
        );
      } catch {
        //
      }
...
export default function TokenHubPage() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data?.profile) {
          const lastClaim = data.profile.last_token_claim
            ? new Date(data.profile.last_token_claim)
            : null;
          if (lastClaim) {
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      ) as { data: CoinStore } | undefined;

      const hasAPT = coinStore && Number(coinStore.data.coin.value) > 0;
  ...
  const claimTask = async (task: any) => {
    ...
    try {
      ...
      const tokenStore = resources.find(
        (r: any) =>
          r.type === `0x1::coin::CoinStore<${task.contract}>`
      ) as { data: CoinStore } | undefined;
      ...
      ) as { data: TokenStore } | undefined;

      if (nftStore) {
        const tokens = nftStore.data.tokens || [];
        hasAsset = tokens.some(
          (t: any) =>
            t.id?.token_data_id?.creator === task.contract ||
            t.id?.token_data_id?.collection === task.contract ||
            t.id?.token_data_id?.name === task.contract
        );
      }
        ...
        >
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <p className="text-yellow-200/80 max-w-xl text-sm md:text-base">
              Hold APT, NFTs, or tokens in your Aptos wallet? Verify ownership
              to earn XP.
            </p>
            <ConnectWallet
          ...
          <div className="space-y-4">
            {tasks.map((task) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-black/70 border border-yellow-500/30 rounded-xl p-5"
              >
                  ...
                  <div>
                    {claimed[task.id] ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-yellow-500 text-black font-semibold hover:bg-yellow-400 disabled:opacity-50"
                      >
                        {claiming === task.id ? (
                          <>
          ...
          {xpAwarded > 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center text-green-400 text-sm mt-6"
            >
